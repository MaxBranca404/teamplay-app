...
export default function ConnectFour({ isSolo, chatId, currentUser }) {
  ...
  // --- LOGICA SOLO ---
  useEffect(() => {
    // La CPU gioca solo se tocca al Giallo (!localRedNext)
  ...
  if (isSolo) {
      ...
      else statusMsg = `Turno: ${localRedNext ? 'Rosso (Tu)' : 'Giallo (CPU)'}`;
  } else if (remoteGame) {
  ...
  // Helper per determinare se è il turno dell'utente corrente (per bloccare i click)
  const isMyTurn = isSolo ? localRedNext : (remoteGame?.turn === currentUser.uid);
  const canInteract = !winner && !isDraw && isMyTurn;
    ...
    <div style={{ textAlign: 'center', maxWidth: '350px', width: '100%' }}>
      ...
      <div style={{ 
        ...
        {board.map((cell, i) => (
          <div key={i} 
            onClick={() => {
                // FIX: Se è singleplayer e NON tocca al rosso (utente), non fare nulla.
                if (isSolo && !localRedNext) return;
                
                isSolo ? handleLocalDrop(i % COLS) : handleRemoteDrop(i % COLS);
            }}
            style={{ 
              width: '100%', 
              aspectRatio: '1/1', 
              ...
              // FIX: Il cursore pointer appare solo se è effettivamente il tuo turno
